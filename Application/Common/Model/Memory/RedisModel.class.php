...
class RedisModel
{
	...
	{
		$this->_HOST = $host ? $host : C('REDIS_HOST');
		$this->_PORT = $port ? $port : C('REDIS_PORT');
		$this->_TIMEOUT = $timeout ? $timeout : C('REDIS_TIMEOUT');
		$this->_DBNAME = $dbName ? $dbName : C('REDIS_DBNAME');
		$this->_CTYPE = $ctype ? $ctype : C('REDIS_CTYPE');
        $this->_AUTH = $auth ? $auth : C('REDIS_PWD');
	 ...
	 */
	public function sAdd($set,$value=null)
	{
	...
	public function setMove($set1, $value=null, $stype=0, $set2=null)
	{
         ...
         * 移出无序集合中某元素
         */
        public function sRem($set, $value=null){
            return $this->_REDIS->sRem($set, $value);
        }
	 ...
	 * @param $value mixed 值
	 */
	public function sIsMember($set, $value=null)
	{
		$return = false;
	 ...
	 * @param $set string 集合名
	 */
	public function sMembers($setKey)
	{
		$return = array();

		$return = $this->_REDIS->sMembers($setKey);
    ...
    public function zRangeByScore($key, $min, $max, $param=array()){
        return $this->_REDIS->zRangeByScore($key, $min, $max, $param);
    }
    ...
    public function zRevRangeByScore($key, $min, $max, $param=array()){
        return $this->_REDIS->zRevRangeByScore($key, $min, $max, $param);
    }

    /**
     * 根据score边界查看有序集合元素数目
     * @param type $key 有序集合key
     * @param type $min score下限
     * @param type $max score上限
     */
    ...
    public function zAdd($key, $score, $val){
        return $this->_REDIS->zAdd($key, $score, $val);
    }
    ...
    public function zRem($key, $member){
        return $this->_REDIS->zRemove($key, $member);
    }
   ...
   /**
     * 通过筛选key获取排名
     */
    public function zRevRank($key,$uid){
        return $this->_REDIS->zRevRank($key, $uid);
    }

    public function __call($method, $args) {
        switch(count($args)){
            case 1:
                return $this->_REDIS->$method($args[0]);
                break;
            case 2:
                return $this->_REDIS->$method($args[0], $args[1]);
                break;
        }
    }
}
